...
var vm = function () {
    ...
    self.activate = function (id) {
        ...
        ajaxHelper(composedUri, 'GET').done(function (data) {
            hideLoading();
            var tags = [];
            for (var x = 0; x < data.Titles.length; x++) {
                var c = data.Titles[x];
                tags.push(c.Name);
            }
            $("#input2").autocomplete({
    ...
    search = function () {
        ...
        var changeUri = 'http://192.168.160.58/netflix/api/search/titles?name=' + $("#input2").val();
        self.moviesList = [];
        self.seriesList = [];
        ajaxHelper(changeUri, 'GET').done(function (data) {
            ...
            for (var info in data) {
                if ((data[info].TypeName) == "Movie")
                    self.moviesList.push(data[info]);
                if ((data[info].TypeName) == "TV Show")
                    self.seriesList.push(data[info]);
            };
        });
    }
    ...
    filter = function () {
        ...
        switch (f) {
            case ("1"):
                ...
                showLoading();
                self.records(self.moviesList);
                self.totalRecords(self.moviesList.length);
                if (self.moviesList.length == 0)
                    hideLoading(),
                        noResults();
                else
                    hideLoading();
            ...
            case ("2"):
                ...
                showLoading();
                self.records(self.seriesList);
                self.totalRecords(self.seriesList.length);
    ...
    function ajaxHelper(uri, method, data) {
        self.error('ERRO'); // Clear error message
        return $.ajax({
            type: method,
            url: uri,
            dataType: 'json',
